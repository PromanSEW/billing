...
public class IabHelper {
	...
	public void dispose() {
		...
		if (mServiceConn != null) {
			...
			try { if (mContext != null) mContext.unbindService(mServiceConn); }
	 ...
	 * {@code onActivityResult} method, at which point you
	...
	public Inventory queryInventory(boolean querySkuDetails, List<String> moreItemSkus) throws IabException {
		if (mDisposed) throw new IabException(BILLING_RESPONSE_RESULT_ERROR, "IabHelper was disposed");
		try {
			...
			throw new IabException(IABHELPER_UNKNOWN_ERROR, "IabHelper is not set up", e);
	...
	public void queryInventoryAsync(final boolean querySkuDetails, final List<String> moreSkus,
		...
		checkNotDisposed();
//		checkSetupDone("queryInventory");
		if (mAsyncOperation.equals("refresh inventory")) flagEndAsync();
	...
	private void consume(Purchase itemInfo) throws IabException {
		...
		try {
			...
			if (mService == null) throw new IabException(IABHELPER_UNKNOWN_ERROR, "Service is null");
	...
	public static String getResponseDesc(int code) {
		...
		if (code < 0 || code >= iab_msgs.length) return code + ":Unknown";
	...
	private int queryPurchases(Inventory inv, String itemType) throws JSONException, RemoteException, NullPointerException {
		// Query purchases
