...
class Security {
	 ...
	 */
	@SuppressWarnings({"PrimitiveArrayArgumentToVarargsMethod", "ConstantConditions"})
	private static boolean verify(PublicKey publicKey, String signedData, String signature) {
		...
		try {
			...
			Method verify = SignatureSpi.class.getDeclaredMethod("engineVerify", byte[].class);
			verify.setAccessible(true);
			if (!(Boolean) verify.invoke(sig, Base64.decode(signature, Base64.DEFAULT))) {
